...
def resolve_current_agency(request: Request, db: Session) -> Agency:
    """Internal helper ensuring agency exists for current tenant using provided session."""
...
def get_current_agency(request: Request, db: Session = Depends(get_db)) -> Agency:  # backward compatible for existing uses
    return resolve_current_agency(request, db)
